...
for (i in 1:100) {
  ...
  wts <- c("0" = 1, "1" = 2)
  RF <- randomForest(REC ~ ., data = dataXL,
                     nodesize = 10, mtry = 4, ntree = 800,
                     keep.forest = TRUE, importance = TRUE, classwt = wts)
  
  # 预测
  prob <- predict(RF, dataCS, type = "prob")[, 2]
  pred_class <- predict(RF, dataCS, type = "response")
  ...
  # 计算AUC
  pred_obj <- prediction(prob, dataCS$REC)
...
# ------------------- 保存模型 -------------------
saveRDS(final_model, "C:/Users/姚竞帆/Desktop/APP2//randomForest_model.rds")

# ------------------- 保存100次实验结果 -------------------
write.csv(results_summary, "C:/Users/姚竞帆/Desktop/APP2/RF_100次实验结果.csv", row.names = FALSE)
...
write.csv(summary_stats, "C:/Users/姚竞帆/Desktop/APP2/RF_模型性能汇总统计.csv", row.names = FALSE)

# ------------------- 输出 -------------------
cat("\n========== 随机森林模型训练完成 ==========\n")
print(summary_stats)
cat("\n模型文件已保存至：C:/Users/姚竞帆/Desktop/APP2/代码+数据/模型/randomForest_model.rds\n")
final_model
